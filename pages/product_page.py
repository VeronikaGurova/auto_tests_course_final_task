...
class ProductPage(BasePage):
    ...
    def should_be_basket_price_equal_to_added_product_price(self):
        ...
        assert product_price == basket_price, "Product price in the basket message does not equal product price on the product page"
    
    # Negative checks
    def should_not_be_basket_message(self):
        assert self.is_not_element_present(*ProductPageLocators.BASKET_MESSAGE_NAME), "Success message is presented, but should not be"
    def success_message_should_disappear(self):
        assert self.is_disappeared(*ProductPageLocators.BASKET_MESSAGE_NAME), "Success message is not desappeared, but should be"